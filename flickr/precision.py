...
def combine_embeddings(image_vectors, reference_vectors):
    """Concatenate image and reference embeddings"""
    return np.concatenate((image_vectors, reference_vectors), axis=1)
...
def evaluate_combined_embeddings(original_file, reduced_file, k_values=[5, 10, 15]):
    ...
    original_reference_vecs = np.array([parse_vector(vec) for vec in df_original['reference_embedding']])
    reduced_reference_vecs = np.array([parse_vector(vec) for vec in df_reduced['reference_embedding']])

    # Remove any None values
    valid_indices = [i for i, (img, cap_orig, cap_red) in
                    enumerate(zip(original_image_vecs, original_reference_vecs, reduced_reference_vecs))
                    if img is not None and cap_orig is not None and cap_red is not None]

    original_image_vecs = original_image_vecs[valid_indices]
    original_reference_vecs = original_reference_vecs[valid_indices]
    reduced_reference_vecs = reduced_reference_vecs[valid_indices]

    # Combine embeddings
    original_combined = combine_embeddings(original_image_vecs, original_reference_vecs)
    reduced_combined = combine_embeddings(original_image_vecs, reduced_reference_vecs)
...
if __name__ == "__main__":
    ...
    results = evaluate_combined_embeddings('embeddings.csv', '360.csv')
    reduced_dim = 360
