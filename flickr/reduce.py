...
def process_embeddings(input_file: str, n_components: int) -> pd.DataFrame:
    ...
    reference_embeddings = np.vstack(df['reference_embedding'].apply(parse_embedding))
    ...
    reference_reduced, reference_pca, reference_variance = apply_pca(reference_embeddings, n_components)
    print(f"Reference embeddings explained variance: {reference_variance:.4f}")

    # Create output DataFrame
    results_df = pd.DataFrame({
        'image_embedding': [embedding.tolist() for embedding in image_reduced],
        'reference_embedding': [embedding.tolist() for embedding in reference_reduced]
    })
...
if __name__ == "__main__":
    ...
    OUTPUT_FILE = os.path.join(DATA_FOLDER, "25.csv")
    ...
    sample_reference = np.array(results['reference_embedding'].iloc[0])
    print(f"Reduced image embedding shape: {sample_image.shape}")
    print(f"Reduced reference embedding shape: {sample_reference.shape}")
