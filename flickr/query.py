...
def load_embeddings(filepath: str) -> Tuple[np.ndarray, np.ndarray, pd.DataFrame]:
    ...
    reference_embedding = np.vstack([parse_embedding(emb) for emb in df['reference_embedding']])
    
    return image_embeddings, reference_embedding, df


def reduce_dimensions(reference_embedding: np.ndarray, dimensions: List[int]) -> Dict[int, np.ndarray]:
    ...
    for dim in dimensions:
        ...
        if dim == 512:
            reduced_embeddings[dim] = reference_embedding
            continue
            
        # Apply PCA
        pca = PCA(n_components=dim)
        reduced = pca.fit_transform(reference_embedding)
        reduced_embeddings[dim] = reduced
        
    return reduced_embeddings


def create_composite_vectors(image_embeddings: np.ndarray, reference_embedding: np.ndarray) -> np.ndarray:
    """Create composite vectors by concatenating image and caption embeddings"""
    return np.hstack((image_embeddings, reference_embedding))
...
def main():
    ...
    n_iterations = 10  # Number of iterations to run for each query
    
    # Load data
    image_embeddings, reference_embedding, _ = load_embeddings(input_file)
