...
def load_data():
    """Load the original 512-dim and 64-dim image embeddings"""
    print("Loading 512-dim embeddings from embedding.csv...")
    df_512 = pd.read_csv('data/embedding.csv')
    
    print("Loading 64-dim image embeddings from 64.csv...")
    df_64 = pd.read_csv('data/64.csv')
    ...
    image_64 = np.vstack(df_64['image_embedding'].apply(parse_embedding))
    
    print(f"Loaded embeddings - Image 512: {image_512.shape}, Caption 512: {caption_512.shape}, Image 64: {image_64.shape}")
    
    return image_512, caption_512, image_64
...
def evaluate_precision(image_512, caption_512, image_64, reduced_captions, k_values=[5, 10, 15]):
    ...
    for dim, reduced_caption in reduced_captions.items():
        # Create reduced combined embeddings (64 + reduced)
        reduced_combined = combine_embeddings(image_64, reduced_caption)
...
def save_precision_results(results, output_file='data/precision_img64.csv'):
...
def plot_precision_curves(results, output_dir='data'):
    ...
    # Save the figure
    output_path = os.path.join(output_dir, 'precision_plots_img64.png')
...
def plot_curve_fits(dimensions, precision_values, curve_results, k=10, output_dir='data'):
    ...
    # Customize plot
    plt.title(f'Precision@{k} - All Curve Fits (64-dim Image Vectors)', fontsize=16)
    ...
    # Save the figure
    output_path = os.path.join(output_dir, f'precision_at_{k}_curves_img64.png')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
...
def plot_derivatives(dimensions, x_smooth, derivatives, k=10, output_dir='data'):
    ...
    # Customize plot
    plt.title(f'Rate of Change Analysis - Precision@{k} (64-dim Image Vectors)', fontsize=16)
    plt.xlabel('Reduced Caption Dimension', fontsize=14)
    ...
    # Save the figure
    output_path = os.path.join(output_dir, f'precision_at_{k}_derivatives_img64.png')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
...
def plot_optimal_range(dimensions, precision_values, curve_results, optimal_ranges, k=10, output_dir='data'):
    ...
    # Customize plot
    plt.title(f'Precision@{k} with Optimal Dimension Range (64-dim Image Vectors)', fontsize=16)
    plt.xlabel('Reduced Caption Dimension', fontsize=14)
    plt.ylabel('Precision', fontsize=14)
    plt.grid(True, alpha=0.3)
    plt.legend(fontsize=12)
    ...
    # Save the figure
    output_path = os.path.join(output_dir, f'optimal_range_k{k}_img64.png')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
...
def main():
    try:
        ...
        # Load the data
        image_512, caption_512, image_64 = load_data()
        ...
        precision_results = evaluate_precision(image_512, caption_512, image_64, reduced_captions)
