...
def load_data():
    """Load the original 512-dim and 128-dim image embeddings"""
    print("Loading 512-dim embeddings from embedding.csv...")
    df_512 = pd.read_csv('data/embedding.csv')
    
    print("Loading 128-dim image embeddings from 128.csv...")
    df_128 = pd.read_csv('data/128.csv')
    ...
    image_128 = np.vstack(df_128['image_embedding'].apply(parse_embedding))
    
    print(f"Loaded embeddings - Image 512: {image_512.shape}, Caption 512: {caption_512.shape}, Image 128: {image_128.shape}")
    
    return image_512, caption_512, image_128
...
def evaluate_precision(image_512, caption_512, image_128, reduced_captions, k_values=[5, 10, 15]):
    ...
    for dim, reduced_caption in reduced_captions.items():
        # Create reduced combined embeddings (128 + reduced)
        reduced_combined = combine_embeddings(image_128, reduced_caption)
...
def save_precision_results(results, output_file='data/precision_img128.csv'):
...
def plot_precision_curves(results, output_dir='data'):
    ...
    # Save the figure
    output_path = os.path.join(output_dir, 'precision_plots_img128.png')
...
def plot_curve_fits(dimensions, precision_values, curve_results, k=10, output_dir='data'):
    ...
    # Customize plot
    plt.title(f'Precision@{k} - All Curve Fits (128-dim Image Vectors)', fontsize=16)
    ...
    # Save the figure
    output_path = os.path.join(output_dir, f'precision_at_{k}_curves_img128.png')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
...
def plot_derivatives(dimensions, x_smooth, derivatives, k=10, output_dir='data'):
    ...
    # Customize plot
    plt.title(f'Rate of Change Analysis - Precision@{k} (128-dim Image Vectors)', fontsize=16)
    plt.xlabel('Reduced Caption Dimension', fontsize=14)
    ...
    # Save the figure
    output_path = os.path.join(output_dir, f'precision_at_{k}_derivatives_img128.png')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
...
def plot_optimal_range(dimensions, precision_values, curve_results, optimal_ranges, k=10, output_dir='data'):
    ...
    # Customize plot
    plt.title(f'Precision@{k} with Optimal Dimension Range (128-dim Image Vectors)', fontsize=16)
    plt.xlabel('Reduced Caption Dimension', fontsize=14)
    plt.ylabel('Precision', fontsize=14)
    plt.grid(True, alpha=0.3)
    plt.legend(fontsize=12)
    ...
    # Save the figure
    output_path = os.path.join(output_dir, f'optimal_range_k{k}_img128.png')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
...
def main():
    try:
        ...
        # Load the data
        image_512, caption_512, image_128 = load_data()
        ...
        precision_results = evaluate_precision(image_512, caption_512, image_128, reduced_captions)
