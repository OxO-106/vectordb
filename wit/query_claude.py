...
def load_embeddings(filepath: str) -> Tuple[np.ndarray, np.ndarray, pd.DataFrame]:
    ...
    caption_embeddings = np.vstack([parse_embedding(emb) for emb in df['caption_embedding']])
    
    return image_embeddings, caption_embeddings, df


def reduce_dimensions(caption_embeddings: np.ndarray, dimensions: List[int]) -> Dict[int, np.ndarray]:
    ...
    for dim in dimensions:
        ...
        if dim == 512:
            reduced_embeddings[dim] = caption_embeddings
            continue
            
        # Apply PCA
        pca = PCA(n_components=dim)
        reduced = pca.fit_transform(caption_embeddings)
        reduced_embeddings[dim] = reduced
        
    return reduced_embeddings


def create_composite_vectors(image_embeddings: np.ndarray, caption_embeddings: np.ndarray) -> np.ndarray:
    """Create composite vectors by concatenating image and caption embeddings"""
    return np.hstack((image_embeddings, caption_embeddings))
...
def main():
    ...
    # Load data
    image_embeddings, caption_embeddings, _ = load_embeddings(input_file)
