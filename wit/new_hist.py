...
for i, (label, title) in enumerate(zip(labels, titles)):
    ...
    x_ticks = np.arange(0, max(data['dimension']) + 1, x_tick_interval)
    ...
    # Save the figure
    fig.savefig(f'histogram_precision_{k_value}_full.png', dpi=600, bbox_inches='tight')
    
    plt.close(fig)

# Create histograms with zoomed y-axis (0.9 to 1.02)
for i, (label, title) in enumerate(zip(labels, titles)):
    # Create a new figure
    fig, ax = plt.subplots(figsize=(8, 4))
    ...
    # Set title
    k_value = label.split('_')[-1]
    ax.set_title(f'Precision@{k_value} Histogram (Zoomed)')
    
    # Set y-axis to zoomed range
    ...
    # Save the figure
    fig.savefig(f'histogram_precision_{k_value}_zoomed.png', dpi=600, bbox_inches='tight')
