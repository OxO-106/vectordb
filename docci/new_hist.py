...
data = data[(data['dimension'] >= 272) & (data['dimension'] <= 512)]
...
for i, (label, title) in enumerate(zip(labels, titles)):
    ...
    x_ticks = np.arange(288, max(data['dimension']) + 1, x_tick_interval)
    ...
    # Save the figure
    fig.savefig(f'docci_histogram_precision_{k_value}.png', dpi=600, bbox_inches='tight')
