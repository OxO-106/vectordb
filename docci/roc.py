...
def parse_vector(vec_str):
    ...
    try:
        if isinstance(vec_str, str):
            # Handle string formatting variations
            ...
            try:
                ...
                # If JSON fails, try regex-based parsing
                # Extract all floating point numbers with regex
                matches = re.findall(r'-?\d+\.\d+(?:e[+-]?\d+)?', vec_str)
                if matches:
...
def main():
    ...
    # Parse original vectors based on the new column names
    print("Parsing original vectors...")
    original_image_vecs = np.array([parse_vector(vec) for vec in df_original['image_embedding']])
    original_caption_vecs = np.array([parse_vector(vec) for vec in df_original['description_embedding']])
    ...
    if len(original_image_vecs) > 0:
        print(f"First image vector shape: {original_image_vecs[0].shape}")
        print(f"First caption vector shape: {original_caption_vecs[0].shape}")
        print(f"Number of valid data points: {len(original_image_vecs)}")
