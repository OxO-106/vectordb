...
def process_dataset(image_folder: str, caption_file: str, color_entropy: bool = True) -> pd.DataFrame:
    ...
    for idx, row in df.iterrows():
        try:
            image_path = os.path.join(image_folder, row['image_file'])  # Use image_file from CSV
            ...
            text_entropy = calculate_text_entropy(row['description'])  # Use description from CSV
...
if __name__ == "__main__":
    ...
    DATA_FOLDER = "data"
    IMAGE_FOLDER = "img"
    CAPTION_FILE = os.path.join(DATA_FOLDER, "data500.csv")
