...
def process_embeddings(data_file: str, output_file: str, column_suffix: str = '') -> None:
    ...
    for idx, row in df.iterrows():
        try:
            ...
            description_embedding = parse_embedding(row['description_embedding'])

            if image_embedding is not None and description_embedding is not None:
                image_embedding_entropy = calculate_embedding_entropy(image_embedding)
                description_embedding_entropy = calculate_embedding_entropy(description_embedding)

                result_row = {
                    f'image_embedding_entropy{column_suffix}': image_embedding_entropy,
                    f'description_embedding_entropy{column_suffix}': description_embedding_entropy
                }
    ...
    print(f"\nDescription Embedding Entropy{column_suffix}:")
    print(results_df[f'description_embedding_entropy{column_suffix}'].describe())
...
if __name__ == "__main__":
    ...
    COLUMN_SUFFIX = "_16"  # Change this if needed (e.g., "_reduced", "_50", etc.)
